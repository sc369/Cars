...
months.forEach((month) => {
    (sortedMonths[month] = (sortedMonths[month] + 1) || 1)
})

const bestMonth = Object.keys(sortedMonths).reduce((a, b) => {
    return sortedMonths[a] >sortedMonths[b] ? a : b})
console.log(bestMonth)


// get agent names
const agentNames = cars.map(car => car.sales_agent.first_name + ` ${car.sales_agent.last_name}`)
cars.forEach((car) => {
car.sales_agent
car.gross_profit
})

//iterate names and create new object with names and number of sales
let sortedAgents = {}
...
let sortedAP = {}
const arrAgentProfits = cars.map(car => [car.sales_agent.first_name + ` ${car.sales_agent.last_name}`, car.gross_profit])
arrAgentProfits.forEach((miniArray) => {
    // if agent name does not exist as key, create it
if (sortedAP.hasOwnProperty(miniArray[0]) === false) {
   sortedAP[miniArray[0]] = 0
}
// add each item of profit to correct agent
sortedAP[miniArray[0]] += miniArray[1]
})
const mostProfit = Object.keys(sortedAP).reduce((a, b) => {
    return sortedAP[a] > sortedAP[b] ? a : b})
console.log(mostProfit)

//create hashtable for car model sales
const models = cars.map(car => car.vehicle.model)
let sortedModels = {}
models.forEach((model) => {
  if (sortedModels.hasOwnProperty(model) === false) {
      sortedModels[model] = 1
  } else {
sortedModels[model]++
  }
})
const mostPopularModel = Object.keys(sortedModels).reduce((a, b) => {
return sortedModels[a] > sortedModels[b] ? a : b}) 
console.log(mostPopularModel)

//create hashtable for credit providers
const sortedCPs = {}
const creditProviders = cars.map(car => car.credit.credit_provider)
creditProviders.forEach(CP => {
    (sortedCPs[CP] = (sortedCPs[CP] + 1) || 1)
})

const mostLoans = Object.keys(sortedCPs).reduce((a, b) => {
    return sortedCPs[a] > sortedCPs[b] ? a : b})
    console.log(mostLoans)
    // console.log(sortedCPs)
